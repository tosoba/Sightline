...
class OpenGLRenderer {
  ...
  private val oglFatalErrorsSharedFlow =
    MutableSharedFlow<Unit>(extraBufferCapacity = 64, onBufferOverflow = BufferOverflow.DROP_OLDEST)
  ...
  fun attachInputPreview(preview: Preview, previewStub: ViewStub) {
    preview.setSurfaceProvider(executor) { surfaceRequest: SurfaceRequest ->
      ...
      val streamStateObserver =
        PreviewStreamStateObserver(camera.cameraInfoInternal, previewStreamStateFlow, renderSurface)
   ...
   * Surface.ROTATION_0. For most phones, this will be a portrait orientation, whereas some tablets
   ...
   *
   * Because the camera sensor (or crop of the camera sensor) may have a different aspect ratio than
   * the Surface that is meant to display it, we also want to fit the image from the camera so the
   * entire Surface is filled. This generally requires scaling the input texture and cropping pixels
   * from either the width or height. We call this transform "center-crop" and is equivalent to the
   * ScaleType with the same name in ImageView.
   */
  @WorkerThread
