...
internal object Math3D {
   ...
   * angle Z separately, as it is a local angle, not an universal one (We want to move our head, not
   * the whole universe)
   *
  ...
  /**
   * * Calculates de relative position of a point getting the camera position as 0,0,0. Returns the
   *   result in meters Check this article:
   *   http://gis.stackexchange.com/questions/2951/algorithm-for-offsetting-a-latitude-longitude-by-some-amount-of-meters
   *
   ...
   * @param inCamPos Input parameter. Current position of the camera, z is latitude in degrees, y is
   *   in meters, x is longitude in degrees
   * @param outRelativePosInMeters Output parameter. Relative distance of the point to the camera. X
   *   and Z are converted to meters
   */
   ...
   *
   * @param inRelativePos Input parameter. The coordinates of a point relative to camera position in
   *   meters
  ...
  fun getRelativeRotation(inRelativePos: Vector3, inCamTrig: Trig3, outRelativeRotPos: Vector3) {
    outRelativeRotPos.x =
      (inCamTrig.yCos * (inCamTrig.zSin * inRelativePos.y + inCamTrig.zCos * inRelativePos.x) -
        inCamTrig.ySin * inRelativePos.z)
    outRelativeRotPos.y =
      (inCamTrig.xSin *
        (inCamTrig.yCos * inRelativePos.z +
          inCamTrig.ySin * (inCamTrig.zSin * inRelativePos.y + inCamTrig.zCos * inRelativePos.x)) +
